...
しかし、以下のような課題があります。
- グラフ書式設定が Excel 画面から作成したグラフの設定と異なる
- 技術報告やレポート用に書式設定が必要

matplotlibのグラフの書式設定や、VBAによるグラフの書式設定に関する情報は多くヒットしますが、  
Python で Excel グラフの書式設定までを自動化するための参考記事は少なく、  
設定する場合にも、多くの設定が必要で手軽に書式設定できるパッケージがないのが現状です。  
そのため、グラフ書式設定までを自動化し、作業を効率化するためのコードを作成しました。
