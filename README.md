## はじめに
本リポジトリの主要コードは、**Pythonで技術資料向けのエクセルグラフの体裁を効率的に整える**ことを目的としています。  
...
## 背景
openpyxl や xlwings を用いることで、Python から Excel グラフを作成することができます。  
しかし、以下のような課題があります。
...
そのため、**グラフ書式設定までを自動化プログラムに組み込み**、作業効率の向上を図ることを目的とした汎用コードを作成しました。
...
- chart_type = "bar" で集合縦棒グラフ(xlColumnClustered)が表示されますが、  
  try構文の例外発生より軸の設定・軸のフォント設定でエラーとなります。
  
- 引数のchart_type = "line"で、片方の系列設定のchart_type = "bar"にすると、棒+折れ線グラフ の組み合わせグラフも表示できます。
