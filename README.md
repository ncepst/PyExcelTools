...
また、`template`関数があればメイン関数不要で、手軽にフォーマットを適用できますが、  
`.crtx`ファイルから書式設定のパラメータを**直接見ること、編集することはできません。**  
本リポジトリのコードは数値的に Excel グラフを制御できることがメリットであると考えています。　　
また、以下のような書式設定パラメータを一覧化して管理することができます。
