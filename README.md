...
ライブラリは xlwings と win32com を使います。

## 背景
openpyxl や xlwings により Excelグラフを作成することができます。  
しかし、以下のような課題があります。
- Excel画面上で手動作成したグラフと比較して、書式設定が一致しない場合が多い
- 技術報告書やレポート用途では、一定の体裁を整えるために多くの書式設定が必要となる

matplotlibのグラフ書式設定や、VBAを用いたグラフ書式設定に関する情報は比較的多く見られますが、  
Python で Excel グラフの書式設定までを自動化するための参考記事が少ないように思われます。  
また、細かな書式設定を行うには多数の設定が必要となり、手軽に書式設定できるパッケージがないことが課題だと考えました。  
そのため、グラフ書式設定までを自動化プログラムに組み込み、作業の効率化を図ることを目的とする汎用コードを作成しました。
