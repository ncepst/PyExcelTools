...
- `title`, `x_title`, `y_title`,`y2_title`, `legend`には文字列を入力できる他に、`False`で無効化を指定できます。  
  `frame_color`には整数(RGB値)を入力できる他に、`False`で外枠の無効化を指定できます。  

- `legend`の仕様   
  `legend="TR+bw+fb"`のようにポジションとオプションを指定できます。順不同で複数指定可能です。  
  `legend="right"`では、プロットエリアの右に凡例を配置します。オプションも利用可能です。
   ```
    Position:  (プロットエリア基準)
...
- 引数で `chart_type = "line"`を指定し、そのうち一部の系列の chart_type を "bar" にすると、  
  棒グラフ+折れ線グラフの組み合わせグラフ を表示できます。(グリッドと軸の設定はできません)
...
また、`template`関数があればメイン関数を使わず、手軽にテンプレートを適用できますが、  
`.crtx`ファイルから**書式設定のパラメータを直接確認、編集することはできません。**

本リポジトリのコードでは、  
●　数値的に Excel グラフを制御できること  
●　書式設定パラメータを一覧化し、コード上で管理できること  
が利点と考えています。
...
```
### 補足
- 透明度(0～1)の`alpha`は`matplotlib`由来です。  
...
- スタイル(`style`)は、以下の指定に対応しています。  
 `"line+marker"`,`"line"`(ラインのみ),`"marker"`(マーカーのみ), `"dash"`(破線), `"dash+marker"`など    
  また、`matplotlib`由来の表記`"--"`や`"-."`も使用可能です。  
  (破線は上記2種類のみ対応しています)  
- 関数の引数が多いため、同じ引数を二度渡さないよう注意してください。  
  (二重指定すると `SyntaxError` になります)
- 系列の色指定については`RGB(r,g,b)`の他に、以下の文字列でも指定できます。
```python
COLOR_NAME_TO_RGB = {
              "blue": RGB(68,114,196),
